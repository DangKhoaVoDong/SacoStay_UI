...
/** Thang cỡ chữ tiêu đề: ELITE=10 · PRO=8 · LITE=6 · BASIC=4 */
const TIER_TITLE_SCALE: Record<VipTier, number> = {
  vip3: 10,
  vip2: 8,
  vip1: 6,
  free: 4
...
/** Sidebar bản đồ — tỉ lệ scale 10:8:6:4 (+10px so với bản trước: 25/23/21/19) */
const SIDEBAR_SIZE_BY_SCALE: Record<number, string> = {
  10: 'text-[21px] font-black uppercase leading-tight tracking-tight',
  8: 'text-[19px] font-bold uppercase leading-tight',
  6: 'text-[17px] font-bold uppercase leading-tight',
  4: 'text-[15px] font-medium leading-snug normal-case'
};

/** Sidebar map — cỡ chữ cân với cột hẹp, ELITE > PRO > LITE > BASIC rõ ràng. */
export function getVipTierSidebarTitleClass(tier?: VipTier | string, selected = false): string {
  if (selected) return 'text-[#FF9F43] font-bold text-sm leading-tight';
  const t = resolveTier(tier);
  const scale = TIER_TITLE_SCALE[t];
  const size = SIDEBAR_SIZE_BY_SCALE[scale] ?? SIDEBAR_SIZE_BY_SCALE[4];
  return `${size} ${TIER_COLOR[t]}`;
}

/**
 * Tiêu đề tin — gói càng cao chữ càng to, đậm, IN HOA (trừ BASIC).
 * compact=true: thẻ danh sách /rooms (giữ cỡ hiện tại, tỉ lệ scale 10:8:6:4).
 */
export function getVipTierTitleClass(tier?: VipTier | string, compact = false): string {
  ...
  if (compact) {
    switch (t) {
      case 'vip3':
        return `text-2xl sm:text-3xl font-black uppercase leading-tight ${TIER_COLOR.vip3} tracking-tight`;
      case 'vip2':
        return `text-xl sm:text-2xl font-bold uppercase leading-tight ${TIER_COLOR.vip2}`;
      case 'vip1':
        return `text-base sm:text-lg font-bold uppercase leading-tight ${TIER_COLOR.vip1}`;
      default:
  ...
  switch (t) {
    case 'vip3':
      return `text-2xl md:text-4xl font-black uppercase leading-snug ${TIER_COLOR.vip3}`;
    case 'vip2':
      return `text-2xl md:text-3xl font-bold uppercase leading-snug ${TIER_COLOR.vip2}`;
    case 'vip1':
      return `text-lg md:text-xl font-bold uppercase leading-snug ${TIER_COLOR.vip1}`;
    default:
